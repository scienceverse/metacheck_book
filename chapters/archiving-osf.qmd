...
The easiest way to do this is the hosted Shiny app at [https://shiny.ieis.tue.nl/metacheck_osf/](https://shiny.ieis.tue.nl/metacheck_osf/), which needs no installation. The rest of this chapter explains how to do the same thing from R, which gives you more control over the download.
...
development version is not available as a pre-compiled binary from
R-universe, so install it directly from GitHub, which builds the package from
source and therefore needs a compiler (Rtools on Windows, the Xcode
command-line tools on macOS, the standard development tools on Linux):
