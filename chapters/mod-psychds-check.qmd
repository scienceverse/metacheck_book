...
This module is still being developed and is in neither the released version of Metacheck nor the `dev` development version. Everything in this chapter — the `psychds_check` module, `convert_psychds()`, `psychds_validate()`, `convert_codebook()` and `convert_behaverse()` — describes work that is not yet part of the package, so the code here will not run against either version you can install.
...
What [data_check](mod-data-check.qmd) already does on its own is the groundwork this module builds on: it classifies every file by type and assigns each one to a study group, which is what decides where a file belongs in a Psych-DS layout. The placement itself, and the conversion, are the parts still to come.
:::
