...
Metacheck is under active development. Installing directly from [GitHub](https://github.com/scienceverse/metacheck) gives you the very latest version, which often has new modules or functions, but may also have more bugs. The R-universe install above is built from the same source and is updated automatically, so for most users it is the better choice; install from GitHub only if you specifically need a change that has not been built yet, or a particular development branch.

Installing from GitHub builds Metacheck from source, which requires a compiler toolchain that does not ship with R by default. You therefore need one extra step compared to the R-universe install:
...
With the build tools in place, install the `pak` package and then install Metacheck from GitHub:
...
To install a specific development branch, append it after an `@`:

``` r
pak::pkg_install("scienceverse/metacheck@dev")
```

After installation, you can load the Metacheck library as usual with `library(metacheck)`.
