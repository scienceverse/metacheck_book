...
This manual mostly explains how to use Metacheck from R. If you do not want to install anything, you can instead use the hosted Shiny app at [https://shiny.ieis.tue.nl/metacheck/](https://shiny.ieis.tue.nl/metacheck/): upload a paper and it creates a standard report, with no installation required. The options are more limited than running Metacheck from R — for example, the use of LLMs is limited, you can only view the report rather than store the result, and you cannot set parameters for modules — but it is the easiest way to use Metacheck.
...
Metacheck is under active development. The development version often has new modules or functions before they reach the stable release, but it may also have more bugs. R-universe only ever hosts one build per package name per universe, so the development version is not available as a pre-compiled binary; install it directly from [GitHub](https://github.com/scienceverse/metacheck) instead. Installing this way builds Metacheck from source, which requires a compiler toolchain that does not ship with R by default:
