...
This is also exposed to [reproducibility_check](mod-reproducibility-check.qmd)'s Docker backend, which uses the declared R version (when found) to pick a matching base image — see `docker_use_declared_version` in that chapter.
...
| `$version_pin` | the raw output of `.code_version_pin_check()` (mechanisms found, declared R version(s), and any `renv.lock` package table) — also used internally by `reproducibility_check()`'s Docker backend to pick a matching base image |
